...
for ns in lakehouse spark unity-catalog iceberg trino; do
  ...
  kubectl -n "$ns" create secret generic lakehouse-s3 \
    --from-literal=AWS_ACCESS_KEY_ID='<rustfs-access-key>' \
    --from-literal=AWS_SECRET_ACCESS_KEY='<rustfs-secret-key>' \
...
kubectl -n iceberg create secret generic nessie-db-app \
  ...
  --from-literal=password='<nessie-db-password>' \
  --dry-run=client -o yaml | kubectl apply -f -
...
kubectl -n superset create secret generic superset-db-app \
  ...
  --from-literal=password='<superset-db-password>' \
  --dry-run=client -o yaml | kubectl apply -f -
kubectl -n superset create secret generic superset-env \
  --from-literal=SUPERSET_SECRET_KEY='<superset-secret-key>' \
  --from-literal=DB_USER=superset \
  --from-literal=DB_PASS='<superset-db-password>' \
  --from-literal=SQLALCHEMY_DATABASE_URI='postgresql+psycopg2://superset:<superset-db-password>@superset-db-rw.superset.svc.cluster.local:5432/superset?sslmode=require' \
